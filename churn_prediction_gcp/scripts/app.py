...
def app():
    ...
    with st.form('Input',clear_on_submit=True):
        uploaded_file = st.file_uploader('Upload Your Test or Training data (Retraining Needs to be Enabled)',type='csv')
            ...
            if retrain_option and uploaded_file!= None:
                try:
                    with st.spinner("Please Wait while we retrain the model...."):
                        ...
                        dataset = create_dataset_artifact(bucket_name,processed_data,uploaded_file.name,uploaded_file.name)
                        job , dataset = initialize_job(dataset,'classification')
                        current_training_jobs , finished_pipelines = check_running_jobs(project_id,'us-central1')
